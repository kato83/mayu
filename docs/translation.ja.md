...
| `chunking.enabled` | bool | `false` | 小型モデル向けテキスト分割の有効化 |
| `chunking.strategy` | string | `"auto"` | 分割戦略: `auto`, `sentence`, `markdown` |
| `chunking.max_chars` | int | `500` | チャンクあたりの最大文字数目安 |

## チャンキング（小型/ローカルモデル向け）

小型のローカル LLM（1B パラメータ未満など）は、長いテキストでタイムアウトしたり翻訳品質が低下することがあります。チャンキング機能はテキストを小さな塊に分割してから LLM に送信します。
...
| 戦略 | 動作 |
|------|------|
| `auto`（デフォルト） | テキストが markdown かどうかを判定し（見出し、コードブロック、リスト等）、markdown なら markdown 分割、そうでなければ文単位分割にフォールバック |
| `sentence` | 常に文の区切り（`. ` + 大文字、段落区切り）で分割 |
| `markdown` | 常に markdown として解析 — ブロック境界（段落、見出し、リストグループ）で分割。フェンスドコードブロックは翻訳せずそのまま保持 |

### 仕組み

1. 入力テキストを選択した戦略に基づいてチャンクに分割
2. コードブロック（` ``` `）は翻訳不要としてマークし、そのまま保持
3. 翻訳対象の各チャンクを個別に LLM へ送信（短い入力 = 高速レスポンス、タイムアウトリスク低減）
4. 翻訳されたチャンクを順番に再結合し、元のドキュメント構造を維持

### 使用が推奨されるケース

- 長い脆弱性説明（details は 2000 文字以上になることがある）でタイムアウトが発生する場合
- 小型モデル（1B パラメータ未満）で長い入力に対して出力品質が低下する場合
- API コール数が増えてもリクエストごとのレイテンシを低減したい場合

> [!NOTE]
> チャンキングを有効にすると LLM API コール数が増加します（フィールドあたり1回ではなくチャンクあたり1回）。
> リクエスト単価課金のクラウド API ではコスト増になる可能性があります。ローカルモデルでは、
> 全体のスループットと引き換えにチャンクごとの信頼性が向上します。
