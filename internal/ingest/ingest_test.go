...
func TestBulkImportAll(t *testing.T) {
	// Read real test data
	data1, err := os.ReadFile("../../testdata/GO-2024-2687.json")
	if err != nil {
		t.Fatalf("read test data: %v", err)
	...
	// Create zip with ecosystem-prefixed paths (top-level all.zip format)
	zipData := createTestZip(t, map[string]string{
		"Go/GO-2024-2687.json": string(data1),
		"Go/GO-2023-1840.json": string(data2),
	})

	// ecosystems.txt content
	ecosystemsTxt := "Go\nnpm\nPyPI\n"
	...
	if err != nil {
		t.Fatalf("BulkImportAll failed: %v", err)
	}

	// Verify stats
	if stats.Ecosystem != "all" {
		t.Errorf("Ecosystem = %q, want %q", stats.Ecosystem, "all")
	}
	if stats.Total != 2 {
		t.Errorf("Total = %d, want 2", stats.Total)
	}
	...
	// Verify "all" sync state was updated
	allState, err := s.GetSyncState(ctx, "all")
	if err != nil {
		t.Fatalf("GetSyncState(all) failed: %v", err)
	}
	if allState == nil {
		t.Fatal("sync state for 'all' is nil")
	}
	if allState.RecordCount != 2 {
		t.Errorf("RecordCount(all) = %d, want 2", allState.RecordCount)
	}

	// Verify each ecosystem's sync state was also updated
	for _, eco := range []string{"Go", "npm", "PyPI"} {
		ecoState, err := s.GetSyncState(ctx, eco)
		if err != nil {
			t.Fatalf("GetSyncState(%s) failed: %v", eco, err)
		}
		if ecoState == nil {
			t.Errorf("sync state for %q is nil, want non-nil", eco)
			continue
		}
		if ecoState.LastModifiedAt != allState.LastModifiedAt {
			t.Errorf("LastModifiedAt(%s) = %s, want %s (same as 'all')", eco, ecoState.LastModifiedAt, allState.LastModifiedAt)
		}
		if ecoState.RecordCount != 0 {
			t.Errorf("RecordCount(%s) = %d, want 0", eco, ecoState.RecordCount)
		}
	}
...
func TestBulkImportAll_EcosystemsListFailed_StillSucceeds(t *testing.T) {
	// Read real test data
	data1, err := os.ReadFile("../../testdata/GO-2024-2687.json")
	if err != nil {
		t.Fatalf("read test data: %v", err)
	...
	// Mock HTTP server — ecosystems.txt returns 500
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/all.zip":
			w.Write(zipData)
	...
	// Execute bulk import — should succeed even if ecosystems.txt fails
	ctx := context.Background()
	stats, err := ing.BulkImportAll(ctx)
	if err != nil {
		t.Fatalf("BulkImportAll failed: %v", err)
	...
	// "all" sync state should be updated
	allState, err := s.GetSyncState(ctx, "all")
	if err != nil {
		t.Fatalf("GetSyncState(all) failed: %v", err)
	}
	...
	if allState.RecordCount != 1 {
		t.Errorf("RecordCount(all) = %d, want 1", allState.RecordCount)
	}

	// Per-ecosystem sync states should NOT exist (ecosystems.txt failed)
	goState, err := s.GetSyncState(ctx, "Go")
	if err != nil {
		t.Fatalf("GetSyncState(Go) failed: %v", err)
	}
	if goState != nil {
		t.Error("sync state for 'Go' should be nil when ecosystems.txt fails")
	}
