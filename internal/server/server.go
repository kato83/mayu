...
func (s *Server) routes() http.Handler {
	...
	r.Route("/api/v1/triage", func(r chi.Router) {
		...
		r.Route("/profiles", func(r chi.Router) {
			r.Get("/", s.handleListTriageProfilesAll)
			r.Post("/", s.handleCreateTriageProfile)
			r.Post("/validate", s.handleValidateTriageProfile)
			r.Get("/{name}", s.handleGetTriageProfile)
			r.Put("/{name}", s.handleUpdateTriageProfile)
			r.Delete("/{name}", s.handleDeleteTriageProfile)
		})
