...
# 外部ディレクトリから Web UI を配信（非 embed ビルド時のみ必要）
...
> [!NOTE]
> リリースバイナリおよび `make build-embed` でビルドした場合は Web UI が内蔵されています。
> `mayu serve` だけで `/` に UI が自動配信されるため、追加オプションは不要です。
> `--ui-dir` オプションは embed なしでビルドした場合（単純な `go build`）や、
> 開発時にカスタム UI ビルドを配信したい場合にのみ使用します。
...
> **注意:** この実装は厳密な P30→P1 変換を使用しています。論文中の `P30/30` 近似は高い EPSS スコアに対して不正確なため、採用していません。
...
> **ヒント:** バックフィルは1日あたり約 5-7 MB（約20万CVEスコア）をダウンロードします。2023-03-07 からのフルバックフィルは約860日分です。既にインポート済みの日付は再実行時に自動スキップされます。
...
> **重要:** LEV は確率的な推定値であり、確定的な事実ではありません。脆弱性の優先順位付けには KEV、EPSS、CVSS など他のシグナルと組み合わせて使用してください。
